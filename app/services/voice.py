...
def is_local_tts_voice(voice_name: str):
    """检查是否是本地TTS的声音"""
    return voice_name.startswith("local:")


def get_local_tts_voices() -> list[str]:
    """获取本地TTS的声音列表 (Qwen3-TTS-12Hz-1.7B-VoiceDesign)"""
    # VoiceDesign 模型的 preset speakers
    return [
        "local:charlie-Male",
        "local:emily-Female",
        "local:isabella-Female",
        "local:james-Male",
        "local:jenny-Female",
        "local:jerry-Male",
        "local:mary-Female",
...
def local_tts(
    ...
    """使用本地TTS服务(Qwen3-TTS-WebUI)生成语音"""
    text = text.strip()

    base_url = config.local_tts.get("base_url", "http://127.0.0.1:7000")
    mode = config.local_tts.get("mode", "design")

    # 从voice_name提取speaker (local:charlie -> charlie)
    speaker = voice_name.replace("local:", "").split("-")[0] if voice_name else "emily"

    url = f"{base_url}/generate/{mode}"

    for i in range(3):
        try:
            # Qwen3-TTS-WebUI 的 API 格式
            files = {}
            data = {
                "text": text,
                "speaker": speaker,
            ...
            logger.info(f"start local tts, mode: {mode}, speaker: {speaker}, try: {i + 1}")
            ...
            if response.status_code == 200:
                ...
                if not audio_url:
                    logger.error(f"local tts failed: no audio_url in response")
                    continue

                # 下载音频文件
                audio_full_url = f"{base_url}{audio_url}" if audio_url.startswith("/") else audio_url
                audio_response = requests.get(audio_full_url, timeout=60)
                ...
                logger.error(f"local tts failed with status code {response.status_code}: {response.text}")
...
def tts(
    ...
    if is_azure_v2_voice(voice_name):
        ...
        return local_tts(text, voice_name, voice_rate, voice_file, voice_volume)
