...
with middle_panel:
    with st.container(border=True):
        ...
        if params.video_source == "local":
            # 不限制文件类型，避免 Streamlit bug 导致的上传失败
    ...
    with st.container(border=True):
        ...
        # 当选择本地TTS时，显示配置输入框
        if selected_tts_server == "local-tts":
            saved_local_tts_url = config.local_tts.get("base_url", "http://127.0.0.1:7000")
            local_tts_url = st.text_input(
                tr("Local TTS URL"),
                value=saved_local_tts_url,
                key="local_tts_url_input",
            )

            # 固定使用 design 模式（只加载了 VoiceDesign 模型）
            st.info(tr("Mode: design (Qwen3-TTS-12Hz-1.7B-VoiceDesign)"))

            config.local_tts["base_url"] = local_tts_url
            config.local_tts["mode"] = "design"
