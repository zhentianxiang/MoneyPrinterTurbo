...
def _generate_response(prompt: str) -> str:
    try:
        ...
        if llm_provider == "g4f":
            ...
            # Also skip for openai when using custom base_url (local deployment)
            if llm_provider not in ["pollinations", "ollama"]:
                # If using custom base_url, api_key is optional
                is_custom_url = base_url and not base_url.startswith("https://api.openai.com")
                if not is_custom_url and llm_provider == "openai":
                    if not api_key:
                        raise ValueError(
...
""".strip()
    ...
    def format_response(response):
        ...
        # Remove AI thinking tags like <thinking>...</thinking> or 【thinking】...【/thinking】
